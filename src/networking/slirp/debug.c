...
void sockstats(void) {
        ...
        for (so = tcb.so_next; so != &tcb; so = so->so_next) {
                ...
                while (n < 17)
                        buff[n++] = ' ';
                buff[17] = 0;
                lprint("%s %3d   %15s %5d ", buff, so->s, inet_ntoa(so->so_laddr), ntohs(so->so_lport));
                lprint("%15s %5d %5d %5d\r\n", inet_ntoa(so->so_faddr), ntohs(so->so_fport), so->so_rcv.sb_cc, so->so_snd.sb_cc);
        }
...
void printf_sockstats(void) {
        ...
        for (so = tcb.so_next; so != &tcb; so = so->so_next) {
                ...
                while (n < 17)
                        buff[n++] = ' ';
                buff[17] = 0;
                printf("%s %3d   %15s %5d ", buff, so->s, inet_ntoa(so->so_laddr), ntohs(so->so_lport));
                printf("%15s %5d %5d %5d\r\n", inet_ntoa(so->so_faddr), ntohs(so->so_fport), so->so_rcv.sb_cc, so->so_snd.sb_cc);
        }
