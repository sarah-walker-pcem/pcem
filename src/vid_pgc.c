...
void pgc_recalctimings(pgc_core_t *pgc)
{
	...
	double pixel_clock = pgc->cga_selected ? 25175000.0 : pgc->native_pixel_clock;

	/* Use a fixed 640 columns, like the T3100e */
        disptime = pgc->screenw + 11;
        _dispontime = pgc->screenw * (cpuclock / pixel_clock);
        _dispofftime = (disptime - _dispontime) * (cpuclock / pixel_clock);
        pgc->dispontime  = (int)(_dispontime  * (1 << TIMER_SHIFT));
        pgc->dispofftime = (int)(_dispofftime * (1 << TIMER_SHIFT));
}
	...
	int (*inpbyte)(struct pgc_core_t *pgc, uint8_t *result), double native_pixel_clock)
{
	...
	pgc->pgc_thread = thread_create(pgc_core_thread, pgc);
        ...
        timer_add(pgc_poll, &pgc->vidtime, TIMER_ALWAYS_ENABLED, pgc);
 
	timer_add(pgc_wake_timer, &pgc->wake_timer, &pgc->wake_timer, 
		(void *)pgc);
}
