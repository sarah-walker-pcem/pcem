...
enum { COLORKEY_8, COLORKEY_16, COLORKEY_32 };
...
static uint32_t MIX(voodoo_t *voodoo, uint32_t dest, uint32_t src, uint32_t pattern, int colour_format_src,
                    int colour_format_dest) {
        int rop_nr = 0;
        uint32_t result = 0;
        uint32_t rop;
...
static void PLOT(voodoo_t *voodoo, int x, int y, int pat_x, int pat_y, uint8_t pattern_mask, uint8_t rop, uint32_t src,
                 int src_colorkey) {
        switch (voodoo->banshee_blt.dstFormat & DST_FORMAT_COL_MASK) {
        case DST_FORMAT_COL_8_BPP: {
                uint32_t addr =
                        get_addr(voodoo, x, y, 0,
                                 0); //(voodoo->banshee_blt.dstBaseAddr + x + y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                uint32_t dest = voodoo->vram[addr];
                uint32_t pattern = (voodoo->banshee_blt.command & COMMAND_PATTERN_MONO)
                                           ? ((pattern_mask & (1 << (7 - (pat_x & 7)))) ? voodoo->banshee_blt.colorFore
        ...
        case DST_FORMAT_COL_16_BPP: {
                uint32_t addr = get_addr(
                        voodoo, x * 2, y, 0,
                        0); //(voodoo->banshee_blt.dstBaseAddr + x*2 + y*voodoo->banshee_blt.dst_stride)  & voodoo->fb_mask;
        ...
        case DST_FORMAT_COL_24_BPP: {
                uint32_t addr = get_addr(
                        voodoo, x * 3, y, 0,
                        0); //(voodoo->banshee_blt.dstBaseAddr + x*3 + y*voodoo->banshee_blt.dst_stride)  & voodoo->fb_mask;
                ...
                *(uint32_t *)&voodoo->vram[addr] =
                        (MIX(voodoo, dest, src, pattern, src_colorkey, COLORKEY_32) & 0xffffff) | (dest & 0xff000000);
                voodoo->changedvram[addr >> 12] = changeframecount;
                break;
        }
        case DST_FORMAT_COL_32_BPP: {
                uint32_t addr = get_addr(
                        voodoo, x * 4, y, 0,
                        0); //(voodoo->banshee_blt.dstBaseAddr + x*4 + y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                uint32_t dest = *(uint32_t *)&voodoo->vram[addr];
                uint32_t pattern = (voodoo->banshee_blt.command & COMMAND_PATTERN_MONO)
                                           ? ((pattern_mask & (1 << (7 - (pat_x & 7)))) ? voodoo->banshee_blt.colorFore
                                                                                        : voodoo->banshee_blt.colorBack)
...
static void PLOT_LINE(voodoo_t *voodoo, int x, int y, uint8_t rop, uint32_t pattern, int src_colorkey) {
        ...
        case DST_FORMAT_COL_16_BPP: {
                ...
                *(uint16_t *)&voodoo->vram[addr] =
                        MIX(voodoo, dest, voodoo->banshee_blt.colorFore, pattern, src_colorkey, COLORKEY_16);
                voodoo->changedvram[addr >> 12] = changeframecount;
                break;
        }
        case DST_FORMAT_COL_24_BPP: {
                ...
                *(uint32_t *)&voodoo->vram[addr] =
                        (MIX(voodoo, dest, voodoo->banshee_blt.colorFore, pattern, src_colorkey, COLORKEY_32) & 0xffffff) |
                        (dest & 0xff000000);
                voodoo->changedvram[addr >> 12] = changeframecount;
                break;
        }
        case DST_FORMAT_COL_32_BPP: {
                ...
                *(uint32_t *)&voodoo->vram[addr] =
                        MIX(voodoo, dest, voodoo->banshee_blt.colorFore, pattern, src_colorkey, COLORKEY_32);
                voodoo->changedvram[addr >> 12] = changeframecount;
                break;
        }
        }
...
static void update_src_stride(voodoo_t *voodoo) {
        ...
        case SRC_FORMAT_PACKING_STRIDE:
                ...
                //                voodoo->banshee_blt.host_data_size_dest, bpp, voodoo->banshee_blt.dstSizeX);
                break;
        ...
        case SRC_FORMAT_PACKING_BYTE:
                ...
                //                voodoo->banshee_blt.host_data_size_dest);
                break;
...
static void banshee_do_rectfill(voodoo_t *voodoo) {
        ...
        int pat_y = (voodoo->banshee_blt.commandExtra & CMDEXTRA_FORCE_PAT_ROW0)
                            ? 0
                            : (voodoo->banshee_blt.patoff_y + voodoo->banshee_blt.dstY);
        ...
        //        voodoo->banshee_blt.dstSizeY, voodoo->banshee_blt.dstX, voodoo->banshee_blt.dstY); pclog("clipping: %i,%i ->
        //        %i,%i\n", clip->x_min, clip->y_min, clip->x_max, clip->y_max); pclog("colorFore=%08x\n",
        //        voodoo->banshee_blt.colorFore);
        for (voodoo->banshee_blt.cur_y = 0; voodoo->banshee_blt.cur_y < voodoo->banshee_blt.dstSizeY;
             voodoo->banshee_blt.cur_y++) {
                ...
                if (dst_y >= clip->y_min && dst_y < clip->y_max) {
                        ...
                        for (voodoo->banshee_blt.cur_x = 0; voodoo->banshee_blt.cur_x < voodoo->banshee_blt.dstSizeX;
                             voodoo->banshee_blt.cur_x++) {
                                int pattern_trans = use_pattern_trans ? (pattern_mask & (1 << (7 - (pat_x & 7)))) : 1;

                                if (dst_x >= clip->x_min && dst_x < clip->x_max && pattern_trans)
                                        PLOT(voodoo, dst_x, dst_y, pat_x, pat_y, pattern_mask, rop, voodoo->banshee_blt.colorFore,
                                             COLORKEY_32);
                        ...
                        for (voodoo->banshee_blt.cur_x = 0; voodoo->banshee_blt.cur_x < voodoo->banshee_blt.dstSizeX;
                             voodoo->banshee_blt.cur_x++) {
                                ...
                                if (dst_x >= clip->x_min && dst_x < clip->x_max && pattern_trans) {
                                        ...
                                        case DST_FORMAT_COL_8_BPP: {
                                                uint32_t dst_addr =
                                                        get_addr(voodoo, dst_x, dst_y, 0,
                                                                 0); //(voodoo->banshee_blt.dstBaseAddr + dst_x +
                                                                     //dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                                                uint32_t src = src_p[src_x_real];
                                                uint32_t dest = voodoo->vram[dst_addr];
                                                uint32_t pattern =
                                                        (voodoo->banshee_blt.command & COMMAND_PATTERN_MONO)
                                                                ? ((pattern_mask & (1 << (7 - (pat_x & 7))))
                                                                           ? voodoo->banshee_blt.colorFore
                                                                           : voodoo->banshee_blt.colorBack)
                                                                : voodoo->banshee_blt
                                                                          .colorPattern8[(pat_x & 7) + (pat_y & 7) * 8];
                                        ...
                                        case DST_FORMAT_COL_16_BPP: {
                                                uint32_t dst_addr =
                                                        get_addr(voodoo, dst_x * 2, dst_y, 0,
                                                                 0); // dst_addr = (voodoo->banshee_blt.dstBaseAddr + dst_x*2 +
                                                                     // dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                                                ...
                                                uint32_t pattern =
                                                        (voodoo->banshee_blt.command & COMMAND_PATTERN_MONO)
                                                                ? ((pattern_mask & (1 << (7 - (pat_x & 7))))
                                                                           ...
                                                                           : voodoo->banshee_blt.colorBack)
                                                                : voodoo->banshee_blt
                                                                          .colorPattern16[(pat_x & 7) + (pat_y & 7) * 8];

                                                *(uint16_t *)&voodoo->vram[dst_addr] =
                                                        MIX(voodoo, dest, src, pattern, COLORKEY_16, COLORKEY_16);
                                                voodoo->changedvram[dst_addr >> 12] = changeframecount;
                                                break;
                                        }
                                        case DST_FORMAT_COL_24_BPP: {
                                                uint32_t dst_addr =
                                                        get_addr(voodoo, dst_x * 3, dst_y, 0,
                                                                 0); // dst_addr = (voodoo->banshee_blt.dstBaseAddr + dst_x*3 +
                                                                     // dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                                                ...
                                                uint32_t pattern =
                                                        (voodoo->banshee_blt.command & COMMAND_PATTERN_MONO)
                                                                ? ((pattern_mask & (1 << (7 - (pat_x & 7))))
                                                                           ...
                                                                           : voodoo->banshee_blt.colorBack)
                                                                : voodoo->banshee_blt
                                                                          .colorPattern24[(pat_x & 7) + (pat_y & 7) * 8];

                                                *(uint32_t *)&voodoo->vram[dst_addr] =
                                                        (MIX(voodoo, dest, src, pattern, COLORKEY_32, COLORKEY_32) & 0xffffff) |
                                                        (dest & 0xff000000);
                                                voodoo->changedvram[dst_addr >> 12] = changeframecount;
                                                break;
                                        }
                                        case DST_FORMAT_COL_32_BPP: {
                                                uint32_t dst_addr =
                                                        get_addr(voodoo, dst_x * 4, dst_y, 0,
                                                                 0); // dst_addr = (voodoo->banshee_blt.dstBaseAddr + dst_x*4 +
                                                                     // dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                                                uint32_t src = *(uint32_t *)&src_p[src_x_real];
                                                uint32_t dest = *(uint32_t *)&voodoo->vram[dst_addr];
                                                uint32_t pattern =
                                                ...
                                                *(uint32_t *)&voodoo->vram[dst_addr] =
                                                        MIX(voodoo, dest, src, pattern, COLORKEY_32, COLORKEY_32);
                                                voodoo->changedvram[dst_addr >> 12] = changeframecount;
                                                break;
                                        }
                                        }
                             ...
                             voodoo->banshee_blt.cur_x++) {
                                ...
                                if (dst_x >= clip->x_min && dst_x < clip->x_max && pattern_trans) {
                                        ...
                                        case SRC_FORMAT_COL_1_BPP: {
                                                ...
                                                src_data = (src_byte & (0x80 >> (src_x & 7))) ? voodoo->banshee_blt.colorFore
                                                                                              : voodoo->banshee_blt.colorBack;
                                                if (voodoo->banshee_blt.command & COMMAND_TRANS_MONO)
                                                        transparent = !(src_byte & (0x80 >> (src_x & 7)));
                                                //                                                        pclog(" 1bpp
                                                //                                                        src_byte=%02x src_x=%i
                                                //                                                        src_data=%x
                                                //                                                        transparent=%i\n",
                                                //                                                        src_byte, src_x,
                                                //                                                        src_data, transparent);
                                                break;
                                        }
                                        case SRC_FORMAT_COL_8_BPP: {
                                        ...
                                        if (!transparent)
                                                PLOT(voodoo, dst_x, dst_y, pat_x, pat_y, pattern_mask, rop, src_data,
                                                     src_colorkey);
                                }
                                if (use_x_dir) {
                                        src_x += (voodoo->banshee_blt.command & COMMAND_DX) ? -1 : 1;
...
static void banshee_do_screen_to_screen_blt(voodoo_t *voodoo) {
        //        pclog("screen_to_screen: %08x %08x %08x\n", voodoo->banshee_blt.srcFormat, voodoo->banshee_blt.src_stride,
        //        voodoo->banshee_blt.src_stride_dest);
        //                return;
        for (voodoo->banshee_blt.cur_y = 0; voodoo->banshee_blt.cur_y < voodoo->banshee_blt.dstSizeY;
             voodoo->banshee_blt.cur_y++) {
                ...
                do_screen_to_screen_line(voodoo, &voodoo->vram[src_addr], 1, voodoo->banshee_blt.srcX,
...
static void banshee_do_host_to_screen_blt(voodoo_t *voodoo, int count, uint32_t data) {
        ...
        //                pclog("banshee_do_host_to_screen_blt: data=%08x host_data_count=%i src_stride_dest=%i
        //                host_data_size_dest=%i\n", data, voodoo->banshee_blt.host_data_count,
        ...
        if ((voodoo->banshee_blt.srcFormat & SRC_FORMAT_PACKING_MASK) == SRC_FORMAT_PACKING_STRIDE) {
                ...
                *(uint32_t *)&voodoo->banshee_blt.host_data[voodoo->banshee_blt.host_data_count] = data;
                ...
                if (voodoo->banshee_blt.host_data_count >= last_byte) {
                        ...
                        //                        voodoo->banshee_blt.dstSizeY, voodoo->banshee_blt.srcX);
                        if (voodoo->banshee_blt.cur_y < voodoo->banshee_blt.dstSizeY) {
                                if ((voodoo->banshee_blt.srcFormat & SRC_FORMAT_COL_MASK) == SRC_FORMAT_COL_1_BPP)
                                        do_screen_to_screen_line(
                                                voodoo, &voodoo->banshee_blt.host_data[(voodoo->banshee_blt.srcX >> 3) & 3], 0,
                                                voodoo->banshee_blt.srcX & 7, 0);
                                else
                                        do_screen_to_screen_line(
                                                voodoo, &voodoo->banshee_blt.host_data[voodoo->banshee_blt.srcX & 3], 0, 0, 0);
                                voodoo->banshee_blt.cur_y++;
                                if (voodoo->banshee_blt.cur_y == voodoo->banshee_blt.dstSizeY)
                                        end_command(voodoo);
                        }
                ...
                *(uint32_t *)&voodoo->banshee_blt.host_data[voodoo->banshee_blt.host_data_count] = data;
                ...
                while (voodoo->banshee_blt.host_data_count >= voodoo->banshee_blt.src_stride_dest) {
                        ...
                        if (voodoo->banshee_blt.host_data_count) {
                                ...
                                *(uint32_t *)&voodoo->banshee_blt.host_data[0] =
                                        data >> (4 - voodoo->banshee_blt.host_data_count) * 8;
                        }
                }
        }
}
...
static void do_screen_to_screen_stretch_line(voodoo_t *voodoo, uint8_t *src_p, int src_x, int *src_y) {
        ...
        /*        pclog("banshee_do_screen_to_screen_stretch_blt:\n");
                pclog("  srcXY=%i,%i srcsizeXY=%i,%i\n", voodoo->banshee_blt.srcX, voodoo->banshee_blt.srcY,
           voodoo->banshee_blt.srcSizeX, voodoo->banshee_blt.srcSizeY); pclog("  dstXY=%i,%i dstsizeXY=%i,%i\n",
                     ...
                     voodoo->banshee_blt.cur_x++) {
                        ...
                        if (dst_x >= clip->x_min && dst_x < clip->x_max && pattern_trans) {
                                ...
                                case DST_FORMAT_COL_8_BPP: {
                                        ...
                                        voodoo->vram[dst_addr] = MIX(voodoo, dest, src, pattern, COLORKEY_8, COLORKEY_8);
                                        //                                                pclog("%i,%i : sdp=%02x,%02x,%02x
                                        //                                                res=%02x\n", voodoo->banshee_blt.cur_x,
                                        //                                                voodoo->banshee_blt.cur_y, src, dest,
                                        //                                                pattern, voodoo->vram[dst_addr]);
                                        voodoo->changedvram[dst_addr >> 12] = changeframecount;
                                        break;
                                }
                                case DST_FORMAT_COL_16_BPP: {
                                        uint32_t dst_addr =
                                                get_addr(voodoo, dst_x * 2, dst_y, 0,
                                                         0); //(voodoo->banshee_blt.dstBaseAddr + dst_x*2 +
                                                             //dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                                ...
                                case DST_FORMAT_COL_24_BPP: {
                                        uint32_t dst_addr =
                                                get_addr(voodoo, dst_x * 3, dst_y, 0,
                                                         0); //(voodoo->banshee_blt.dstBaseAddr + dst_x*3 +
                                                             //dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
                                        ...
                                        *(uint32_t *)&voodoo->vram[dst_addr] =
                                                (MIX(voodoo, dest, src, pattern, COLORKEY_32, COLORKEY_32) & 0xffffff) |
                                                (*(uint32_t *)&voodoo->vram[dst_addr] & 0xff000000);
                                        //                                                pclog("%i,%i : sdp=%02x,%02x,%02x
                                        //                                                res=%02x\n", voodoo->banshee_blt.cur_x,
                                        //                                                voodoo->banshee_blt.cur_y, src, dest,
                                        //                                                pattern, voodoo->vram[dst_addr]);
                                ...
                                case DST_FORMAT_COL_32_BPP: {
                                        uint32_t dst_addr =
                                                get_addr(voodoo, dst_x * 4, dst_y, 0,
                                                         0); //(voodoo->banshee_blt.dstBaseAddr + dst_x*4 +
                                                             //dst_y*voodoo->banshee_blt.dst_stride) & voodoo->fb_mask;
...
static void banshee_do_screen_to_screen_stretch_blt(voodoo_t *voodoo) {
        ...
        for (voodoo->banshee_blt.cur_y = 0; voodoo->banshee_blt.cur_y < voodoo->banshee_blt.dstSizeY;
             voodoo->banshee_blt.cur_y++) {
                uint32_t src_addr = get_addr(voodoo, 0, voodoo->banshee_blt.srcY, 1,
                                             voodoo->banshee_blt.src_stride_src); //(voodoo->banshee_blt.srcBaseAddr +
                                                                                  //voodoo->banshee_blt.srcY*voodoo->banshee_blt.src_stride_src)
                                                                                  //& voodoo->fb_mask;
                //                pclog("scale_blit %i %08x  %08x\n", voodoo->banshee_blt.cur_y, src_addr,
                //                voodoo->banshee_blt.command); if ((voodoo->banshee_blt.srcFormat & SRC_FORMAT_COL_MASK) ==
                //                SRC_FORMAT_COL_1_BPP)
                //                        pclog(" srcY=%i src_addr=%08x\n", voodoo->banshee_blt.srcY, src_addr);
                do_screen_to_screen_stretch_line(voodoo, &voodoo->vram[src_addr], voodoo->banshee_blt.srcX,
        ...
        if ((voodoo->banshee_blt.srcFormat & SRC_FORMAT_PACKING_MASK) == SRC_FORMAT_PACKING_STRIDE) {
                ...
                *(uint32_t *)&voodoo->banshee_blt.host_data[voodoo->banshee_blt.host_data_count] = data;
                ...
                if (voodoo->banshee_blt.host_data_count >= last_byte) {
                        ...
                        if (voodoo->banshee_blt.cur_y < voodoo->banshee_blt.dstSizeY) {
                                if ((voodoo->banshee_blt.srcFormat & SRC_FORMAT_COL_MASK) == SRC_FORMAT_COL_1_BPP)
                                        do_screen_to_screen_stretch_line(
                                                voodoo, &voodoo->banshee_blt.host_data[(voodoo->banshee_blt.srcX >> 3) & 3],
                                                voodoo->banshee_blt.srcX & 7, NULL);
                                else
                                        do_screen_to_screen_stretch_line(
                                                voodoo, &voodoo->banshee_blt.host_data[voodoo->banshee_blt.srcX & 3], 0, NULL);
                                voodoo->banshee_blt.cur_y++;
                                if (voodoo->banshee_blt.cur_y == voodoo->banshee_blt.dstSizeY)
                                        end_command(voodoo);
                        }
        ...
        if (dx > dy) /*X major*/
        {
                ...
                while (x != voodoo->banshee_blt.dstX) {
                        ...
                        if (y >= clip->y_min && y < clip->y_max && x >= clip->x_min && x < clip->x_max && pattern_trans)
                                PLOT_LINE(voodoo, x, y, rop, mask ? voodoo->banshee_blt.colorFore : voodoo->banshee_blt.colorBack,
                                          COLORKEY_32);
...
static void banshee_polyfill_continue(voodoo_t *voodoo, uint32_t data) {
        ...
        /*        pclog("   verts now : %03i,%03i    %03i,%03i\n", voodoo->banshee_blt.lx[0], voodoo->banshee_blt.ly[0],
           voodoo->banshee_blt.rx[0], voodoo->banshee_blt.ry[0]); pclog("               %03i,%03i    %03i,%03i\n",
           voodoo->banshee_blt.lx[1], voodoo->banshee_blt.ly[1], voodoo->banshee_blt.rx[1], voodoo->banshee_blt.ry[1]); pclog("
           left  dx=%i dy=%i x_inc=%i error=%i\n",
                ...
                if (y >= clip->y_min && y < clip->y_max) {
                        ...
                        for (x = voodoo->banshee_blt.lx_cur; x < voodoo->banshee_blt.rx_cur; x++) {
                                ...
                                if (x >= clip->x_min && x < clip->x_max && pattern_trans)
                                        PLOT(voodoo, x, y, pat_x, pat_y, pattern_mask, rop, voodoo->banshee_blt.colorFore,
                                             COLORKEY_32);
                        }
                }
...
void voodoo_2d_reg_writel(voodoo_t *voodoo, uint32_t addr, uint32_t val) {
                ...
                switch (voodoo->banshee_blt.command & COMMAND_CMD_MASK) {
                        /*                        case COMMAND_CMD_SCREEN_TO_SCREEN_STRETCH_BLT:
                                                if (voodoo->banshee_blt.bresError0 & BRES_ERROR_USE)
                                                        voodoo->banshee_blt.bres_error_0 =
                           (int32_t)(int16_t)(voodoo->banshee_blt.bresError0 & BRES_ERROR_MASK); else
                                                        voodoo->banshee_blt.bres_error_0 = voodoo->banshee_blt.dstSizeY / 2;
                                                if (voodoo->banshee_blt.bresError1 & BRES_ERROR_USE)
                                                        voodoo->banshee_blt.bres_error_1 =
                           (int32_t)(int16_t)(voodoo->banshee_blt.bresError1 & BRES_ERROR_MASK); else
        ...
        case 0xfc:
                ...
                //                voodoo->banshee_blt.commandExtra, voodoo->banshee_blt.srcColorkeyMin,
                //                voodoo->banshee_blt.srcColorkeyMax);
                switch (voodoo->banshee_blt.command & COMMAND_CMD_MASK) {
                case COMMAND_CMD_SCREEN_TO_SCREEN_BLT:
                        voodoo->banshee_blt.srcXY = val;
                        voodoo->banshee_blt.srcX = ((int32_t)(val << 19)) >> 19;
