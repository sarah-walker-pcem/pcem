...
//        pclog("ati28800_out : %04X %02X  %04X:%04X\n", addr, val, CS,pc);
	...
	switch (addr) {
	case 0x1ce:ati28800->index = val;
		break;
	case 0x1cf:old = ati28800->regs[ati28800->index];
		...
		case 0xbe:
			...
			break;
		case 0xb3:ati_eeprom_write(&ati28800->eeprom, val & 8, val & 2, val & 1);
			break;
		case 0xb6:
			if ((old ^ val) & 0x10)
				svga_recalctimings(svga);
...
//                pclog("ati28800k_out : set port 03DE to %02X at %04X:%04X\n", val, CS, cpu_state.oldpc);
		...
		if (ati28800->get_korean_font_enabled) {
			...
			case 0x10:ati28800->get_korean_font_base = ((val & 0x7F) << 7) | (ati28800->get_korean_font_base & 0x7F);
				break;
			case 8:ati28800->get_korean_font_base = (ati28800->get_korean_font_base & 0x3F80) | (val & 0x7F);
				break;
			case 1:ati28800->get_korean_font_kind = (ati28800->get_korean_font_kind & 0xFF00) | val;
				if (val & 2)
		...
		break;
	default:ati28800_out(oldaddr, val, p);
		break;
	}
}
...
//        if (addr != 0x3da) pclog("ati28800_in : %04X ", addr);
	...
	switch (addr) {
	case 0x1ce:temp = ati28800->index;
		break;
	case 0x1cf:
		switch (ati28800->index) {
		case 0xb7:temp = ati28800->regs[ati28800->index] & ~8;
			if (ati_eeprom_read(&ati28800->eeprom))
				temp |= 8;
			break;

		default:temp = ati28800->regs[ati28800->index];
			break;
		}
		break;
...
//        if (addr != 0x3da) pclog("ati28800_in : %04X ", addr);
	...
	case 0x3DE:
		...
		break;
	default:temp = ati28800_in(oldaddr, p);
		break;
	}
//        if (addr != 0x3da) pclog("%02X  %04X:%04X\n", temp, CS,cpu_state.pc);
	return temp;
...
void ati28800_recalctimings(svga_t *svga) {
	...
	case 0x00:svga->clock = (cpuclock * (double)(1ull << 32)) / 42954000.0;
		break;
	case 0x01:svga->clock = (cpuclock * (double)(1ull << 32)) / 48771000.0;
		break;
	case 0x03:svga->clock = (cpuclock * (double)(1ull << 32)) / 36000000.0;
		break;
	case 0x04:svga->clock = (cpuclock * (double)(1ull << 32)) / 50350000.0;
		break;
	case 0x05:svga->clock = (cpuclock * (double)(1ull << 32)) / 56640000.0;
		break;
	case 0x07:svga->clock = (cpuclock * (double)(1ull << 32)) / 44900000.0;
		break;
	case 0x08:svga->clock = (cpuclock * (double)(1ull << 32)) / 30240000.0;
		break;
	case 0x09:svga->clock = (cpuclock * (double)(1ull << 32)) / 32000000.0;
		break;
	case 0x0A:svga->clock = (cpuclock * (double)(1ull << 32)) / 37500000.0;
		break;
	case 0x0B:svga->clock = (cpuclock * (double)(1ull << 32)) / 39000000.0;
		break;
	case 0x0C:svga->clock = (cpuclock * (double)(1ull << 32)) / 50350000.0;
		break;
	case 0x0D:svga->clock = (cpuclock * (double)(1ull << 32)) / 56644000.0;
		break;
	case 0x0E:svga->clock = (cpuclock * (double)(1ull << 32)) / 75000000.0;
		break;
	case 0x0F:svga->clock = (cpuclock * (double)(1ull << 32)) / 65000000.0;
		break;
	default:break;
	}
