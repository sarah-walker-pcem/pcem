...
//        pclog("  %04X:%04X\n", CS,cpu_state.pc);        
	switch (addr) {
	case 0x3C3:oti037->enable_register = val & 1;
		return;
	case 0x3D4:svga->crtcreg = val /*& 31*/; // fixme: bios wants to set the test bit?
		return;
	case 0x3D5:
	...
	case 0x3DE:oti037->index = val /*& 0x1f*/;
		return;
	case 0x3DF:idx = oti037->index & 0x1f;
		oti037->regs[idx] = val;
		switch (idx) {
		case 0xD:
			if (val & 0x80)
...
//        if (addr != 0x3da && addr != 0x3ba) pclog("oti037_in : %04X ", addr);
	...
	switch (addr) {
	case 0x3C3:temp = oti037->enable_register;
		break;
	case 0x3D4:temp = svga->crtcreg;
		break;
	case 0x3D5:temp = svga->crtc[svga->crtcreg & 31];
		break;
