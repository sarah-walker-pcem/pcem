...
//        pclog("RAMDAC command reg now %02X\n", val);
	switch (val & 0xf0) {
	case 0x00:
	case 0x10:svga->bpp = 8;
		break;

	case 0x20:
	case 0x30:
	case 0x80:
	case 0xa0:svga->bpp = 15;
		break;

	case 0x50:
	case 0x60:
	case 0xc0:svga->bpp = 16;
		break;

	case 0x40:
	case 0x90:
	case 0xe0:svga->bpp = 24;
		break;

	case 0x70:svga->bpp = 32;
		break;
...
//        /*if (CS!=0xC000) */pclog("OUT RAMDAC %04X %02X %i %04X:%04X  %i  %i %i\n",addr,val,ramdac->magic_count,CS,cpu_state.pc, ramdac->rs2, ramdac->rindex, ramdac->windex);
	...
	case 3:ramdac->magic_count = 0;
		break;
	case 0:ramdac->magic_count = 0;
		break;
	case 1:ramdac->magic_count = 0;
		break;

	case 4:ramdac->windex = val;
		ramdac->reg_ff = 0;
		break;
	case 5:sdac_reg_write(ramdac, ramdac->windex & 0xff, val);
		break;
	case 6:sdac_control_write(ramdac, svga, val);
		break;
	case 7:ramdac->rindex = val;
		ramdac->reg_ff = 0;
		break;
	}
	if (!(addr & 4)) {
...
//        /*if (CS!=0xC000) */pclog("IN RAMDAC %04X %04X:%04X %i  %i %i\n",addr,CS,cpu_state.pc, ramdac->rs2, ramdac->rindex, ramdac->windex);
	...
	case 4:return ramdac->windex;
	case 5:return sdac_reg_read(ramdac, ramdac->rindex & 0xff);
	case 6:return ramdac->command;
	case 7:return ramdac->rindex;
	}
	if (!(addr & 4)) {
		if (addr < 2)
