...
static uint8_t flash_read(uint32_t addr, void *p) {
	...
	case CMD_READ_ARRAY:
	default:return flash->array[addr];
	...
	case CMD_READ_STATUS:return flash->status;
	}
}
...
static uint16_t flash_readw(uint32_t addr, void *p) {
	...
	switch (flash->command) {
	case CMD_READ_ARRAY:
	default:return *(uint16_t *)&(flash->array[addr]);

	case CMD_IID:return 0x89 | (flash->flash_id << 8);
...
//        pclog("flash_write : addr=%08x val=%02x command=%02x %04x:%08x\n", addr, val, flash->command, CS, cpu_state.pc);
	...
	default:flash->command = val;
		switch (val) {
		case CMD_CLEAR_STATUS:flash->status = 0;
			break;
		}
	}
}
...
void *intel_flash_init(uint8_t type, uint8_t flash_id) {
	...
	switch (romset) {
	case ROM_REVENGE:strcpy(flash_path, "revenge/");
		break;
	case ROM_PB520R:strcpy(flash_path, "pb520r/");
		break;
	case ROM_ENDEAVOR:strcpy(flash_path, "endeavor/");
		break;
	case ROM_PB570:strcpy(flash_path, "pb570/");
		break;
	case ROM_ZAPPA:strcpy(flash_path, "zappa/");
		break;
	case ROM_P55VA:strcpy(flash_path, "p55va/");
		break;
	case ROM_P55TVP4:strcpy(flash_path, "p55tvp4/");
		break;
	case ROM_430VX:strcpy(flash_path, "430vx/");
		break;
	case ROM_P55T2P4:strcpy(flash_path, "p55t2p4/");
		break;
	case ROM_ITAUTEC_INFOWAYM:strcpy(flash_path, "infowaym/");
		break;
	case ROM_VS440FX:strcpy(flash_path, "vs440fx/");
		break;
	case ROM_GA686BX:strcpy(flash_path, "ga686bx/");
		break;
