...
/*		switch (val & (TCR_MSG | TCR_CD | TCR_IO))
	...
	default:pclog("Bad NCR5380 write %06x %02x\n", addr, val);
	}
...
//                pclog("  bus and status register = %02x\n", temp);
	...
	default:pclog("Bad NCR5380 read %06x\n", addr);
	}
...
static uint8_t t130b_in(uint16_t port, void *p) {
	...
	case 0x1:
	case 0x2:
	case 0x3:temp = lcs6821n_read((port & 7) | 0x3980, scsi);
		break;

	case 0x4:
	case 0x5:temp = lcs6821n_read(0x3900, scsi);
		break;
	...
	case 0xd:
	case 0xe:
	case 0xf:temp = ncr5380_read(port, &scsi->ncr);
		break;
	}
...
static void t130b_out(uint16_t port, uint8_t val, void *p) {
	...
	case 0x1:
	case 0x2:
	case 0x3:lcs6821n_write((port & 7) | 0x3980, val, scsi);
		break;

	case 0x4:
	case 0x5:lcs6821n_write(0x3900, val, scsi);
		break;
	...
	case 0xd:
	case 0xe:
	case 0xf:ncr5380_write(port, val, &scsi->ncr);
		break;
	}
}
...
//                                        output=3;
	...
	default:pclog("DMA callback bad mode %i\n", scsi->ncr.dma_mode);
		break;
	}
