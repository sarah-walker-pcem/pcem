...
//                        pclog("INQUIRY %d  %02x %02x %02x %02x %02x %02x  %i\n", len, cdb[0],cdb[1],cdb[2],cdb[3],cdb[4],cdb[5],  data->hd_id);
	...
	case SCSI_MODE_SENSE_6:len = cdb[4];
...
//                scsi_illegal_field();
	...
	case SCSI_READ_CAPACITY_10:scsi_add_data((data->hdd.sectors >> 24) & 0xff, data);
...
//                pclog("SCSI_MODE_SELECT_6 complete\n");
	...
	case SCSI_FORMAT:bus_state = BUS_CD | BUS_IO;
		break;

	case SCSI_START_STOP_UNIT:bus_state = BUS_CD | BUS_IO;
		break;
	...
	default:pclog("Bad SCSI HD command %02x\n", cdb[0]);
		scsi_hd_illegal(data);
		bus_state = BUS_CD | BUS_IO;
		break;
	}
