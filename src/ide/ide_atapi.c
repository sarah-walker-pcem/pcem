...
void atapi_data_write(atapi_device_t *atapi_dev, uint16_t val) {
	...
	case ATAPI_STATE_COMMAND:atapi_dev->command[atapi_dev->command_pos++] = val & 0xff;
	...
	case ATAPI_STATE_WRITE_DATA_WAIT:atapi_dev->data[atapi_dev->data_write_pos++] = val & 0xff;
...
//        pclog("atapi_process_packet: state=%i\n", atapi_dev->state);
	switch (atapi_dev->state) {
	case ATAPI_STATE_COMMAND:*atapi_dev->atastat = READY_STAT | (*atapi_dev->atastat & ERR_STAT);
...
//                                        pclog("NEXT_PHASE - %x\n", bus_state);
			...
			if (bus_state & BUS_REQ) {
				...
				case 0:atapi_dev->data_read_pos = scsi_dev->get_bytes_required(scsi_data);
...
//                                                        pclog("WRITE_DATA: bytes=%i\n", *atapi_dev->cylinder);
				...
				case BUS_IO:atapi_dev->state = ATAPI_STATE_READ_DATA;
					atapi_dev->data_read_pos = atapi_dev->data_write_pos = 0;
					break;

				case (BUS_IO | BUS_CD):atapi_dev->state = ATAPI_STATE_READ_STATUS;
					break;

				case (BUS_CD | BUS_IO | BUS_MSG):atapi_dev->state = ATAPI_STATE_READ_MESSAGE;
					break;

				default:fatal("NEXT_PHASE - bus state changed %x\n", bus_state);
				}
				break;
			}
		}
...
//                                        pclog("Read message %02x\n", msg);
				...
				switch (msg) {
				case MSG_COMMAND_COMPLETE:atapi_dev->state = ATAPI_STATE_END_PHASE;
					break;
