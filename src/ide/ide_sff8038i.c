...
static void sff_bus_master_next_addr(sff_busmaster_t *busmaster, int channel) {
        ...
        //        pclog("New DMA settings on channel %i - Addr %08X Count %04X EOT %i\n", channel, piix_busmaster[channel].addr,
        //        piix_busmaster[channel].count, piix_busmaster[channel].eot);
}
...
void sff_bus_master_write(uint16_t port, uint8_t val, void *p) {
        ...
        case 0:
                ...
                break;
        case 2:
                busmaster[channel].status =
                        (val & 0x60) | ((busmaster[channel].status & ~val) & 6) | (busmaster[channel].status & 1);
                break;
        case 4:
        ...
        while (transferred < size) {
                if (busmaster[channel].count < (size - transferred) && busmaster[channel].eot)
                        fatal("DMA on channel %i - Read count less than size! Addr %08X Count %04X EOT %i size %i\n", channel,
                              busmaster[channel].addr, busmaster[channel].count, busmaster[channel].eot, size);
                ...
                //                piix_busmaster[channel].addr, piix_busmaster[channel].count, piix_busmaster[channel].eot);
