...
void disc_sector_reset(int drive, int side) { disc_sector_count[drive][side] = 0; }
...
void disc_sector_stop() { disc_sector_state = STATE_IDLE; }
...
void disc_sector_poll() {
        ...
        case STATE_READ_FIND_SECTOR:
                /*                pclog("STATE_READ_FIND_SECTOR: cur_sector=%i cur_byte=%i sector=%i,%i side=%i,%i track=%i,%i
                   period=%i,%i\n", cur_sector, cur_byte, disc_sector_sector, s->r, disc_sector_side, s->h, disc_sector_track,
                   s->c, fdc_get_bitcell_period(), get_bitcell_period());*/
                if (index_count > 1) {
                ...
                if (cur_byte || fdc_get_bitcell_period() != get_bitcell_period() ||
                    !fdd_can_read_medium(disc_sector_drive ^ fdd_swap) || disc_intersector_delay) {
                        advance_byte();
                        break;
                }
        ...
        case STATE_READ_FIND_FIRST_SECTOR:
                ...
                if (cur_byte || !index_count || fdc_get_bitcell_period() != get_bitcell_period() || disc_intersector_delay) {
                        advance_byte();
                        break;
                }
        ...
        case STATE_READ_FIND_NEXT_SECTOR:
                ...
                if (cur_byte || fdc_get_bitcell_period() != get_bitcell_period() || disc_intersector_delay) {
                        advance_byte();
                        break;
                }
        ...
        case STATE_FORMAT_FIND:
                ...
                if (!index_count || fdc_get_bitcell_period() != get_bitcell_period() || disc_intersector_delay) {
                        advance_byte();
                        break;
                }
                if (!(fdd_can_read_medium(disc_sector_drive ^ fdd_swap))) {
