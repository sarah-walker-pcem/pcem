...
int wx_sendmessage(void *window, int type, LONG_PARAM param1, LONG_PARAM param2) {
	switch (type) {
	case WX_CB_ADDSTRING:((wxComboBox *)window)->Append((char *)param2);
		break;
	case WX_CB_SETCURSEL:
		if (param1 >= 0 && param1 < (int)((wxComboBox *)window)->GetCount())
			((wxComboBox *)window)->Select(param1);
		break;
	case WX_CB_GETCURSEL:return ((wxComboBox *)window)->GetCurrentSelection();
		break;
	case WX_CB_GETLBTEXT:strcpy((char *)param2, ((wxComboBox *)window)->GetString(param1).mb_str());
		break;
	...
	case WX_BM_SETCHECK:((wxCheckBox *)window)->SetValue(param1);
		break;
	case WX_BM_GETCHECK:return ((wxCheckBox *)window)->GetValue();
		break;
	case WX_WM_SETTEXT: {
	...
	case WX_WM_GETTEXT:
		...
		break;
	case WX_UDM_SETPOS:((wxSpinCtrl *)window)->SetValue(param2);
		break;
	case WX_UDM_GETPOS:return ((wxSpinCtrl *)window)->GetValue();
