...
static int shader_manager_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
        ...
        case WX_COMMAND: {
                if (wParam == wxID_OK) {
                        ...
                        int ret = !wx_filedialog(hdlg, "Open", shaders[c],
                                                 "GLSL Shaders (*.glslp;*.glsl)|*.glslp;*.glsl|All files (*.*)|*.*", 0, 1, s);
...
int shader_manager_open(void *hwnd) { return wx_dialogbox(hwnd, "ShaderManagerDlg", shader_manager_dlgproc); }
