...
static void ali1429_recalc() {
	...
	for (c = 0; c < 8; c++) {
		...
		if (ali1429_regs[0x13] & (1 << c)) {
			...
			case 0:mem_set_mem_state(base, 0x8000, MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL);
				break;
			case 1:mem_set_mem_state(base, 0x8000, MEM_READ_INTERNAL | MEM_WRITE_EXTERNAL);
				break;
			case 2:mem_set_mem_state(base, 0x8000, MEM_READ_EXTERNAL | MEM_WRITE_INTERNAL);
				break;
			case 3:mem_set_mem_state(base, 0x8000, MEM_READ_INTERNAL | MEM_WRITE_INTERNAL);
				break;
			}
		} else
...
//                pclog("ALI1429 write %02X %02X %04X:%04X %i\n",ali1429_index,val,CS,pc,ins);
		...
		case 0x13:ali1429_recalc();
			break;
		case 0x14:ali1429_recalc();
			break;
		}
	}
}
