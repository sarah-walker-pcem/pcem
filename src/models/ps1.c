...
void ps1mb_init() {
        ...
        rom_init(&ps1_high_rom, "ibmps1es/f80000.bin", 0xf80000, 0x80000, 0x7ffff, 0, MEM_MAPPING_EXTERNAL);
...
static void ps1_m2121_recalc_memory() {
        ...
        mem_set_mem_state(0x00000, 0x80000,
                          (ps1_e0_regs[0] & 0x01) ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                  : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
        /*Enable 512-640kb*/
        mem_set_mem_state(0x80000, 0x20000,
                          (ps1_e0_regs[1] & 0x01) ? (MEM_READ_INTERNAL | MEM_WRITE_INTERNAL)
                                                  : (MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL));
}
...
void ps1mb_m2121_init() {
        ...
        rom_init(&ps1_high_rom, "ibmps1_2121/fc0000.bin", 0xfc0000, 0x40000, 0x3ffff, 0, MEM_MAPPING_EXTERNAL);
        ps1_92 = 0;
        ps1_190 = 0;
...
void ps1mb_m2133_init(void) { io_sethandler(0x0398, 0x0002, ps1_m2133_read, NULL, NULL, ps1_m2133_write, NULL, NULL, NULL); }
