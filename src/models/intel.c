...
uint8_t endeavor_brdconfig(uint16_t port, void *p) {
        ...
        case 0x79:
                ...
                // 0     | Internal CPU Clock Freq. (Switch 6) | 3/2x          | 2x              (Manual has these swapped in one
                // place?) 1     | Soft Off capable power supply       | No            | Yes 2     | On-board Audio present | Yes
                // | No 3     | External CPU clock (Switch 7)       |               | 4     | External CPU clock (Switch 8) | | 5
                // | Setup Disable (Switch 5)            | Enable access | Disable access 6     | Clear CMOS (Switch 4) | Keep
                // values   | Clear values 7     | Password Clear (Switch 3)           | Keep password | Clear password
...
void intel_endeavor_init() { io_sethandler(0x0079, 0x0001, endeavor_brdconfig, NULL, NULL, NULL, NULL, NULL, NULL); }
...
static uint8_t zappa_brdconfig(uint16_t port, void *p) {
        ...
        case 0x79:
                // Bit # | Description                         | Bit = 1       | Bit = 0
                // 0     | Internal CPU Clock Freq. (Switch 6) | 3/2x          | 2x              (Manual has these swapped in one
                // place?) 1     | No Connect                          |               | 2     | No Connect |               | 3 |
                // External CPU clock (Switch 7)       |               | 4     | External CPU clock (Switch 8)       | | 5     |
                // Setup Disable (Switch 5)            | Enable access | Disable access 6     | Clear CMOS (Switch 4) | Keep
                // values   | Clear values 7     | Password Clear (Switch 3)           | Keep password | Clear password
...
void intel_zappa_init() { io_sethandler(0x0079, 0x0001, zappa_brdconfig, NULL, NULL, NULL, NULL, NULL, NULL); }
