...
static void sl82c460_shadow_set(uint32_t base, uint32_t size, int state) {
	switch (state) {
	case 0:mem_set_mem_state(base, size, MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL);
		break;
	case 1:mem_set_mem_state(base, size, MEM_READ_EXTERNAL | MEM_WRITE_INTERNAL);
		break;
	case 2:mem_set_mem_state(base, size, MEM_READ_INTERNAL | MEM_WRITE_EXTERNAL);
		break;
	case 3:mem_set_mem_state(base, size, MEM_READ_INTERNAL | MEM_WRITE_INTERNAL);
		break;
	}
...
//                pclog("SL82C460 write %02x %02x\n", sl82c460.reg_idx, val);
		...
		case 0x18:
		case 0x19:
		case 0x1b:sl82c460_shadow_recalc();
			break;
		}
	}
}
