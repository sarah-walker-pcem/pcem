...
//                pclog("IRQ routing %02x %02x\n", addr, val);
	...
	case REG_SMIREQ:card_sio[addr] &= val;
		return;
	case REG_SMIREQ + 1:card_sio[addr] &= val;
		return;
	}
	card_sio[addr] = val;
}
...
static uint8_t sio_apm_read(uint16_t port, void *p) {
	...
	switch (port) {
	case 0xb2:ret = sio->pm.apmc;
		break;

	case 0xb3:ret = sio->pm.apms;
		break;
	}
