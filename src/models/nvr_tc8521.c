...
void tc8521_loadnvr() {
	...
	oldromset = romset;
	switch (romset) {
	case ROM_T1000: f = nvrfopen("t1000.nvr", "rb");
		break;
	case ROM_T1200: f = nvrfopen("t1200.nvr", "rb");
		break;
	default: return;
	}
	if (!f) {
...
void tc8521_savenvr() {
	FILE *f;
	switch (oldromset) {
	case ROM_T1000: f = nvrfopen("t1000.nvr", "wb");
		break;
	case ROM_T1200: f = nvrfopen("t1200.nvr", "wb");
		break;
	default: return;
	}
