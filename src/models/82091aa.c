...
//                pclog("AIP register write: idx=%02x val=%02x\n", aip.idx, val);
		switch (aip.idx) {
		case AIP_AIPCFG1:aip.regs[AIP_AIPCFG1] = (val & ~0x4e) | (aip.regs[AIP_AIPCFG1] & 0x48);
			break;
		case AIP_AIPCFG2:aip.regs[AIP_AIPCFG2] = val & 0xf8;
			break;
		case AIP_FCFG1:aip.regs[AIP_FCFG1] = val & 0x83;
			fdc_remove();
			if ((aip.regs[AIP_FCFG1] & FCFG1_FDC_ENA) && !(aip.regs[AIP_FCFG1] & FCFG1_FDC_ADDR))
				fdc_add();
			break;
		case AIP_FCFG2:aip.regs[AIP_FCFG2] = (val & 0x0d) | (aip.regs[AIP_FCFG2] & 0x02);
			break;
		case AIP_PCFG1:aip.regs[AIP_PCFG1] = val & 0xef;
			lpt1_remove();
			if (aip.regs[AIP_PCFG1] & PCFG1_PP_ENA) {
				switch (aip.regs[AIP_PCFG1] & PCFG1_PP_ADDR) {
				case PCFG1_PP_ADDR_378:lpt1_init(0x378);
					break;
				case PCFG1_PP_ADDR_278:lpt1_init(0x278);
					break;
				case PCFG1_PP_ADDR_3BC:lpt1_init(0x3bc);
					break;
				}
			}
			break;
		case AIP_PCFG2:aip.regs[AIP_PCFG2] = (val & 0x0d) | (aip.regs[AIP_PCFG2] & 0x22);
			break;
		case AIP_SACFG1:aip.regs[AIP_SACFG1] = val & 0x9f;
			serial1_remove();
			if (aip.regs[AIP_SACFG1] & SxCFG1_ENA) {
				...
				case SxCFG1_ADDR_3F8: base = 0x3f8;
					break;
				case SxCFG1_ADDR_2F8: base = 0x2f8;
					break;
				case SxCFG1_ADDR_220: base = 0x220;
					break;
				case SxCFG1_ADDR_228: base = 0x228;
					break;
				case SxCFG1_ADDR_238: base = 0x238;
					break;
				case SxCFG1_ADDR_2E8: base = 0x2e8;
					break;
				case SxCFG1_ADDR_338: base = 0x338;
					break;
				case SxCFG1_ADDR_3E8: base = 0x3e8;
					break;
				}
				switch (aip.regs[AIP_SACFG1] & SxCFG1_IRQ) {
				case SxCFG1_IRQ_3: irq = 3;
					break;
				case SxCFG1_IRQ_4: irq = 4;
					break;
				}
				serial1_set(base, irq);
			}
			break;
		case AIP_SACFG2:aip.regs[AIP_SACFG2] = (val & 0x01d) | (aip.regs[AIP_SACFG2] & 0x02);
			break;
		case AIP_SBCFG1:aip.regs[AIP_SBCFG1] = val & 0x9f;
			serial2_remove();
			...
			break;
		case AIP_SBCFG2:aip.regs[AIP_SACFG2] = (val & 0x01d) | (aip.regs[AIP_SACFG2] & 0x02);
			break;
		case AIP_IDECFG:aip.regs[AIP_IDECFG] = val & 0x07;
			break;
		}
	}
}
