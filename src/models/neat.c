...
void neat_write(uint16_t port, uint8_t val, void *priv) {
	switch (port) {
	case 0x22:neat_index = val;
		break;

	case 0x23:neat_regs[neat_index] = val;
		switch (neat_index) {
	...
	case 0x8209:
	case 0xC208:
	case 0xC209:neat_emspage[port >> 14] = (neat_emspage[port >> 14] & 0x180) | (val & 0x7F);
		break;
	}
}
...
uint8_t neat_read(uint16_t port, void *priv) {
	switch (port) {
	case 0x22:return neat_index;

	case 0x23:return neat_regs[neat_index];
	}
	return 0xff;
}
