...
void neat_writeems(uint32_t addr, uint8_t val) { ram[(neat_emspage[(addr >> 14) & 3] << 14) + (addr & 0x3FFF)] = val; }

uint8_t neat_readems(uint32_t addr) { return ram[(neat_emspage[(addr >> 14) & 3] << 14) + (addr & 0x3FFF)]; }
