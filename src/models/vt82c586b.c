...
static void vt82c586b_pci_isa_bridge_write(int addr, uint8_t val) {
        /*Read-only addresses*/
        if ((addr < 4) || (addr == 5) || (addr == 6) || (addr >= 8 && addr < 0x40) || (addr == 0x49) || (addr == 0x4b) ||
            (addr >= 0x51 && addr < 0x54) || (addr >= 0x5d && addr < 0x60) || (addr >= 0x68 && addr < 0x6a) || (addr >= 0x71))
                return;
...
static void vt82c586b_ide_write(int addr, uint8_t val) {
        /*Read-only addresses*/
        if ((addr < 4) || (addr == 5) || (addr == 8) || (addr >= 0xa && addr < 0x0d) || (addr >= 0x0e && addr < 0x20) ||
            (addr >= 0x22 && addr < 0x3c) || (addr >= 0x3d && addr < 0x40) || (addr >= 0x54 && addr < 0x60) ||
...
static void vt82c586b_usb_write(int addr, uint8_t val) {
        /*Read-only addresses*/
        if ((addr < 4) || (addr == 5) || (addr == 6) || (addr >= 8 && addr < 0xd) || (addr >= 0xe && addr < 0x20) ||
            (addr >= 0x22 && addr < 0x3c) || (addr >= 0x3e && addr < 0x40) || (addr >= 0x42 && addr < 0x44) ||
...
static void vt82c586b_power_write(int addr, uint8_t val) {
        /*Read-only addresses*/
        if ((addr < 0xd) || (addr >= 0xe && addr < 0x40) || (addr == 0x43) || (addr == 0x48) || (addr >= 0x4a && addr < 0x50) ||
            (addr >= 0x54))
                return;
        ...
        case 0x41:
                ...
                if (vt82c586b.power_regs[0x41] & ACPI_IO_ENABLE)
                        io_sethandler(vt82c586b.power.io_base, 0x0100, power_reg_read, NULL, NULL, power_reg_write, NULL, NULL,
                                      NULL);
                break;
...
uint8_t vt82c586b_read_sysctrl(uint16_t port, void *p) { return vt82c586b.sysctrl; }
