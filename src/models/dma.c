...
static uint8_t dma_ps2_read(uint16_t addr, void *priv) {
	...
	case 0x1a:
		...
		case 3:
			...
			case 0:temp = dma_c->ac & 0xff;
				dma_ps2.byte_ptr = 1;
				break;
			case 1:temp = (dma_c->ac >> 8) & 0xff;
				dma_ps2.byte_ptr = 2;
				break;
			case 2:temp = (dma_c->ac >> 16) & 0xff;
				dma_ps2.byte_ptr = 0;
				break;
			}
			break;
		...
		default:fatal("Bad XFR Read command %i channel %i\n", dma_ps2.xfr_command, dma_ps2.xfr_channel);
		}
		break;
	}
...
//        pclog("Write PS2 DMA %04X %02X %04X:%04X\n",addr,val,CS,cpu_state.pc);
	...
	switch (addr) {
	case 0x18:dma_ps2.xfr_channel = val & 0x7;
...
//                        case 0xb: output = 3; break;
	...
	case 0x1a:
		...
		case 2: /*Address*/
			switch (dma_ps2.byte_ptr) {
			case 0:dma_c->ac = (dma_c->ac & 0xffff00) | val;
				dma_ps2.byte_ptr = 1;
				break;
			case 1:dma_c->ac = (dma_c->ac & 0xff00ff) | (val << 8);
				dma_ps2.byte_ptr = 2;
				break;
			case 2:dma_c->ac = (dma_c->ac & 0x00ffff) | (val << 16);
				dma_ps2.byte_ptr = 0;
				break;
			}
			dma_c->ab = dma_c->ac;
		...
		default:fatal("Bad XFR command %i channel %i val %02x\n", dma_ps2.xfr_command, dma_ps2.xfr_channel, val);
		}
		break;
	}
}
