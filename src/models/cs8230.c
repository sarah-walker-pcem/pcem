...
//        pclog("shadow_control: addr=%08x size=%04x state=%02x\n", addr, size, state);
	switch (state) {
	case 0x00:mem_set_mem_state(addr, size, MEM_READ_INTERNAL | MEM_WRITE_INTERNAL);
		break;
	case 0x01:mem_set_mem_state(addr, size, MEM_READ_EXTERNAL | MEM_WRITE_INTERNAL);
		break;
	case 0x10:mem_set_mem_state(addr, size, MEM_READ_INTERNAL | MEM_WRITE_EXTERNAL);
		break;
	case 0x11:mem_set_mem_state(addr, size, MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL);
		break;
	}
	flushmmucache_nopc();
}
...
static uint8_t cs8230_read(uint16_t port, void *p) {
	...
	if (port & 1) {
		...
		case 0x28:
		case 0x29:
		case 0x2a:ret = cs8230.regs[cs8230.idx];
			break;
		}
	}
