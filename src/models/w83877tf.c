...
enum { CHIP_W83877F, CHIP_W83877TF };
...
static void w83877tf_write_reg(w83877tf_t *w83877tf, int index, uint8_t val) {
        ...
        case 0x16:
                ...
                if (val & 0x01) {
                        io_sethandler(0x03f0, 0x0002, w83877tf_read_3fx, NULL, NULL, w83877tf_write_3fx, NULL, NULL,
                                      &w83877tf_global);
                        w83877tf->unlock_key = (w83877tf->regs[0xc] & (1 << 5)) ? 0x87 : 0x86;
                } else {
                        io_sethandler(0x0250, 0x0003, w83877tf_read_25x, NULL, NULL, w83877tf_write_25x, NULL, NULL,
                                      &w83877tf_global);
...
static void w83877tf_write_3fx(uint16_t port, uint8_t val, void *p) {
        ...
        case 0x3f0: /*Enable/index register*/
                if (val == 0xaa) {
                        ...
                        w83877tf->key[0] = val;
                        w83877tf->enable_regs =
                                (w83877tf->key[0] == w83877tf->unlock_key && w83877tf->key[1] == w83877tf->unlock_key);
...
void w83877f_init(uint16_t base, uint8_t key) { w83877_common_init(CHIP_W83877F, base, key); }
void w83877tf_init(uint16_t base, uint8_t key) { w83877_common_init(CHIP_W83877TF, base, key); }
