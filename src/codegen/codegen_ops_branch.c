...
static int ropJO_common(codeblock_t *block, ir_data_t *ir, uint32_t dest_addr, uint32_t next_pc) {
	...
	case FLAGS_SUB8:
	case FLAGS_DEC8:jump_uop = uop_CMP_JNO_DEST(ir, IREG_flags_op1_B, IREG_flags_op2_B);
		break;

	case FLAGS_SUB16:
	case FLAGS_DEC16:jump_uop = uop_CMP_JNO_DEST(ir, IREG_flags_op1_W, IREG_flags_op2_W);
		break;

	case FLAGS_SUB32:
	case FLAGS_DEC32:jump_uop = uop_CMP_JNO_DEST(ir, IREG_flags_op1, IREG_flags_op2);
		break;

	case FLAGS_UNKNOWN:
	default:uop_CALL_FUNC_RESULT(ir, IREG_temp0, VF_SET);
...
static int ropJNO_common(codeblock_t *block, ir_data_t *ir, uint32_t dest_addr, uint32_t next_pc) {
	...
	case FLAGS_SUB8:
	case FLAGS_DEC8:jump_uop = uop_CMP_JO_DEST(ir, IREG_flags_op1_B, IREG_flags_op2_B);
		break;

	case FLAGS_SUB16:
	case FLAGS_DEC16:jump_uop = uop_CMP_JO_DEST(ir, IREG_flags_op1_W, IREG_flags_op2_W);
		break;

	case FLAGS_SUB32:
	case FLAGS_DEC32:jump_uop = uop_CMP_JO_DEST(ir, IREG_flags_op1, IREG_flags_op2);
		break;
...
static int ropJB_common(codeblock_t *block, ir_data_t *ir, uint32_t dest_addr, uint32_t next_pc) {
	...
	case FLAGS_UNKNOWN:
	default:uop_CALL_FUNC_RESULT(ir, IREG_temp0, CF_SET);
		if (do_unroll)
			jump_uop = uop_CMP_IMM_JNZ_DEST(ir, IREG_temp0, 0);
		else
			jump_uop = uop_CMP_IMM_JZ_DEST(ir, IREG_temp0, 0);
...
static int ropJS_common(codeblock_t *block, ir_data_t *ir, uint32_t dest_addr, uint32_t next_pc) {
	...
	case FLAGS_UNKNOWN:
	default:uop_CALL_FUNC_RESULT(ir, IREG_temp0, NF_SET);
		if (do_unroll)
			jump_uop = uop_CMP_IMM_JNZ_DEST(ir, IREG_temp0, 0);
		else
			jump_uop = uop_CMP_IMM_JZ_DEST(ir, IREG_temp0, 0);
...
static int ropJL_common(codeblock_t *block, ir_data_t *ir, uint32_t dest_addr, uint32_t next_pc) {
	...
	case FLAGS_UNKNOWN:
	default:uop_CALL_FUNC_RESULT(ir, IREG_temp0, NF_SET_01);
