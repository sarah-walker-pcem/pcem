...
#define ropF_arith_mem(name, load_uop)                                                                                           \
        uint32_t ropFADD##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,             \
                               uint32_t op_pc) {                                                                                 \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
        ...
        uint32_t ropFCOM##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,             \
                               uint32_t op_pc) {                                                                                 \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFCOMP##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFDIV##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,             \
                               uint32_t op_pc) {                                                                                 \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFDIVR##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFMUL##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,             \
                               uint32_t op_pc) {                                                                                 \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFSUB##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,             \
                               uint32_t op_pc) {                                                                                 \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFSUBR##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
...
ropF_arith_mem(s, uop_MEM_LOAD_SINGLE) ropF_arith_mem(d, uop_MEM_LOAD_DOUBLE)

#define ropFI_arith_mem(name, temp_reg)                                                                                          \
        uint32_t ropFIADD##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFICOM##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFICOMP##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,           \
                                 uint32_t op_pc) {                                                                               \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFIDIV##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFIDIVR##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,           \
                                 uint32_t op_pc) {                                                                               \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFIMUL##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFISUB##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,            \
                                uint32_t op_pc) {                                                                                \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        uint32_t ropFISUBR##name(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32,           \
                                 uint32_t op_pc) {                                                                               \
                x86seg *target_seg;                                                                                              \
                                                                                                                                 \
                uop_FP_ENTER(ir);                                                                                                \
        ...
        ropFI_arith_mem(l, IREG_temp0) ropFI_arith_mem(w, IREG_temp0_W)

                uint32_t
        ropFABS(codeblock_t *block, ir_data_t *ir, uint8_t opcode, uint32_t fetchdat, uint32_t op_32, uint32_t op_pc) {
        uop_FP_ENTER(ir);
