...
void codegen_ir_compile(ir_data_t *ir, codeblock_t *block) {
        ...
        if (codegen_unroll_count) {
                ...
                for (unroll_count = 1; unroll_count < codegen_unroll_count; unroll_count++) {
                        ...
                        //                        pclog("Unroll from %i to %i, offset %i - iteration %i\n", codegen_unroll_start,
                        //                        ir->wr_pos, offset, unroll_count);
        ...
        for (c = 0; c < ir->wr_pos; c++) {
                ...
                if ((uop->type & UOP_MASK) == (UOP_MOV_IMM & UOP_MASK) && reg_is_native_size(uop->dest_reg_a) &&
                    !codegen_reg_is_loaded(uop->dest_reg_a) &&
                    reg_version[IREG_GET_REG(uop->dest_reg_a.reg)][uop->dest_reg_a.version].refcount <= 0) {
                        ...
                        if ((uop->type & UOP_MASK) == (UOP_MOV & UOP_MASK) &&
                            reg_version[IREG_GET_REG(uop->src_reg_a.reg)][uop->src_reg_a.version].refcount <= 1 &&
