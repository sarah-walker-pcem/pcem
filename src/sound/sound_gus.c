...
//                pclog("Setting ad command %02X %02X %p\n", val, gus->adcommand, &gus->adcommand);
	...
	case 0x240:gus->midi_loopback = val & 0x20;
...
//                        pclog("IRQ %i DMA %i\n", gus->irq, gus->dma);
		...
		case 2:gus->gp2 = val;
			break;
		case 3:gus->gp1_addr = val;
			break;
		case 4:gus->gp2_addr = val;
			break;
		case 5:gus->usrr = 0;
			break;
		case 6:break;
	...
	case 0x246:gus->ad_status |= 0x08;
		...
		break;
	case 0x24a:gus->sb_2xa = val;
		break;
	case 0x24c:gus->ad_status |= 0x10;
		if (gus->sb_ctrl & 0x20) {
			if (gus->sb_nmi)
				nmi = 1;
			else if (gus->irq != -1)
	...
	case 0x24d:gus->sb_2xc = val;
		break;
	case 0x24e:gus->sb_2xe = val;
		break;
	case 0x24f:gus->reg_ctrl = val;
		break;
	}
}
...
//                pclog("GUS RAM read %05X %02X\n",gus->addr,val);
	...
	case 0x24b:
		...
		case 2:val = gus->gp2;
			break;
		case 3:val = gus->gp1_addr;
			break;
		case 4:val = gus->gp2_addr;
			break;
		}
		break;

	case 0x24c:val = gus->sb_2xc;
...
//                pclog("Read ad_status %02X\n", gus->ad_status);
	...
	case 0x249:gus->ad_status &= ~0x01;
		nmi = 0;
	case 0x389:val = gus->ad_data;
		break;

	case 0x24A:val = gus->adcommand;
